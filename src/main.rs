...
use joystick::{read_joystick_as_direction, setup_port};
use piston_window::*;
use tiles::Tiles;
...
fn main() {
    ...
    let mut port = setup_port("/dev/tty.usbmodem143101");
    ...
    while let Some(event) = window.next() {
        // println!("{:?}", joystick::read_lines(&mut port));
        if let Some(direction) = read_joystick_as_direction(&mut port) {
            game_state.safely_change_snake_direction(direction);
        }
